...
sql_conn = sql_conn('postgres')
## get market cap data frame
def market_cap_df(pg_conn = sql_conn):
...
def reddit_agg_by_day(pg_conn = sql_conn):
    sql = """select num_posts, name, created 
        from coin.reddit_post_by_day_agg"""
    df = pd.read_sql(sql,pg_conn)
    return df

def reddit_trends_df(pg_conn=sql_conn):
...
df_rt = reddit_trends_df(sql_conn)
df_mc = market_cap_df(sql_conn)
coin_list = list(df_mc['name'].unique())
df_red_agg = reddit_agg_by_day(sql_conn)
...
def update_total_mc(coin_select, date_filter):
    ...
    date_cutoff = df_mc.max()['insert_timestamp'] - pd.Timedelta(days=date_filter)
    data = [{
...
def filter_df(df,coin_select,date_filter):
...
def filter_reddit(df,coin_select,date_filter):
    df_stg = df[df['name'].isin(coin_select)]
